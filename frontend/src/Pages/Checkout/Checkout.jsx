...
const Checkout = () => {
  ...
  useEffect(() => {
    const getCupancode = async () => {
      try {
        const res = await axios.get("https://goyat-trading-co-1.onrender.com/api/all-vouchers");
        if (res.status === 200) {
    ...
    const getApiData = async () => {
      try {
        const res = await axios.get(`https://goyat-trading-co-1.onrender.com/api/get-user/${userId}`);
        if (res.status === 200) {
  ...
  const calculateCartSummary = async (cartItems) => {
    ...
    if (tempSubtotal >= 6000) {
      ...
      try {
        const response = await axios.get("https://goyat-trading-co-1.onrender.com/api/all-pincode");
  ...
  const handleConfirmOrder = async (event) => {
    ...
    Swal.fire({
      ...
      if (result.isConfirmed) {
        ...
        try {
          const res = await axios.post("https://goyat-trading-co-1.onrender.com/api/checkout", checkoutData);
          console.log(res);
          if (res.status === 201) {
            if (paymentMethod === "Online") {
              ...
              const options = {
                ...
                handler: async (response) => {
                  const verifyResponse = await axios.post("https://goyat-trading-co-1.onrender.com/api/payment/verify", {
